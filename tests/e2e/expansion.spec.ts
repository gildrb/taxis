...
test("keeps imported SVG masks vector-based through gradients, transforms, and project restore", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /Masked Stripes/ }).click();
...
test("replays animation phases exactly and exports the paused frame", async ({ page }) => {
  await page.goto("/");
  await page.getByRole("button", { name: /Radial Rays/ }).click();
  await chooseOption(page, "Mode", "Gradient");
  ...
  await expect(canvas).not.toHaveAttribute("data-phase", "0.25");
  await page.getByRole("button", { name: "Pause", exact: true }).click();
  const stoppedPhase = Number(await canvas.getAttribute("data-phase"));
  await expect(phase).toHaveValue(String(stoppedPhase));
  const svg = await exportText(page, "SVG");
  await expectSvgMatchesCanvas(page, svg);
  const text = await exportText(page, "Export Project");
  const project = JSON.parse(text) as PatternProject;
  expect(project.params.animationPhase).toBe(stoppedPhase);
  ...
  await expect(canvas).toHaveAttribute("data-phase", String(stoppedPhase));
  await expect(page.getByRole("button", { name: "Play", exact: true })).toBeVisible();
  await expectSvgMatchesCanvas(page, svg);
...
test("lets agents inspect, edit, render, and restore the exact same scene as the UI", async ({ page }) => {
  ...
  const edited = await page.evaluate(() => {
    ...
    api.setParams({ preset: "radial", sourceMode: "ignore", colorMode: "gradient", patternScaleX: 0.7, patternScaleY: 1.15, rotation: 28 });
...
test("lets agents import SVG source geometry without operating file-picker UI", async ({ page }) => {
  ...
  const result = await page.evaluate(async () => {
    ...
    const scene = window.taxis.setParams({ preset: "stripes", sourceMode: "mask", cellSize: 12, lineWidth: 0.5 });
...
test("imports a scene while playing without stale-state cancellation", async ({ page }) => {
  await page.goto("/");
  await page.waitForFunction(() => Boolean(window.taxis));
  const saved = await page.evaluate(() => window.taxis.setParams({ preset: "radial", sourceMode: "ignore", animation: "pulse", animationPhase: 0.25 }));
  await page.getByRole("button", { name: "Play", exact: true }).click();
  await expect(page.locator("canvas")).not.toHaveAttribute("data-phase", "0.25");
  await page.evaluate(async (scene) => { await window.taxis.setScene(scene); }, saved);
  await expect(page.locator("canvas")).toHaveAttribute("data-phase", "0.25");
  await expect(page.getByRole("button", { name: "Play", exact: true })).toBeVisible();
});
...
test("composes masked vector forms in a padded grid with independent cell overrides", async ({ page }) => {
  ...
  const output = await page.evaluate(() => {
    const scene = window.taxis.setParams({
      width: 600, height: 400, preset: "stripes", sourceMode: "ignore", cellSize: 14,
...
test("keeps independently rotated masks and rings vector-exact under global anisotropic scale", async ({ page }) => {
  ...
  const svg = await page.evaluate(() => {
    window.taxis.setParams({
      width: 560, height: 420, preset: "rings", sourceMode: "ignore", cellSize: 10,
...
test("edits whole-pattern clips and repeat spacing through product controls", async ({ page }) => {
  await page.goto("/");
  await page.waitForFunction(() => Boolean(window.taxis));
  await page.evaluate(() => { window.taxis.setParams({ preset: "stripes", sourceMode: "ignore" }); });
